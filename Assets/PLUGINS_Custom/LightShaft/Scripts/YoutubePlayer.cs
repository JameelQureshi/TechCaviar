...
public class YoutubePlayer : RequestResolver
{
    ...
    public GameObject[] VideoPlayersDuplicate;
    public AudioSource AudioOnOFF;
    ...
    private void ShowLoading()
    {
        if (loadingContent != null)
        {
            ...
            AudioOnOFF.volume = 0f;
            VideoPlayersDuplicate = GameObject.FindGameObjectsWithTag("TargetPlayer");
            for (int i = 0; i < VideoPlayersDuplicate.Length; i++)
            {
                VideoPlayersDuplicate[i].transform.GetChild(1).transform.gameObject.SetActive(true);
            }
        }
    }
    ...
    private void HideLoading()
    {
        if (loadingContent != null)
        {
            ...
            AudioOnOFF.volume = 1f;
            VideoPlayersDuplicate = GameObject.FindGameObjectsWithTag("TargetPlayer");
            for (int i = 0; i < VideoPlayersDuplicate.Length; i++)
            {
                VideoPlayersDuplicate[i].transform.GetChild(1).transform.gameObject.SetActive(false);
            }
        }
    }
