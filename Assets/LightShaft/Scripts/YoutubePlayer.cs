...
public class YoutubePlayer : MonoBehaviour
{
    ...
    [Space]
    [Header("Cutom Playlist playback")]
    public bool multipleUrls;
    ...
    [Header("If video not work you can use an alternative format")]
    public bool alternativeFormat = false;
    ...
    [Space]
    [Header("If you enable this you can ignore the bellow settings , you will get the url to pass to the player only.")]
    [Space]
    ...
    private void UrlsLoaded()
    {
        ...
        else
        {
            ...
            foreach (VideoInfo info in videoInfos)
            {
                VideoType t = (alternativeFormat) ? VideoType.Mp4 : VideoType.WebM;
        ...
        if (!finishedCalled)
        {
            ...
            if (!loadYoutubeUrlsOnly)
            {
                if (videoPlayer.isPrepared)
                {
                    ...
                    if (multipleUrls && autoPlayNextVideo)
                    {
    ...
    public static void WriteLog(string filename, string c)
    {
        ...
        Debug.Log("log write here: "+ filePath);
    }
